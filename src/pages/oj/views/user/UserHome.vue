<template>
  <div class="container">
    ...
    <div class="avatar-container">
      <img class="avatar" :src="profile.IMGBuffer || '/static/image/defaultAvatar.png'"/>
    </div>
    <Card :padding="100">
      <div v-if="profile.id">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      init () {
        // 不能将this.$route.query.id在data中赋值 当路由变化时第二次的值还是先前的
      ...
      getSolvedProblems () {
        ...
        Promise.all(promiseRequestQueue).then(res => {
          this.solovedProbles = res[0].data.data.sort()
          this.notSolovedProblems = res[1].data.data.sort()
          this.spinShow = false
        }).catch(_ => {
          this.spinShow = false
        })
