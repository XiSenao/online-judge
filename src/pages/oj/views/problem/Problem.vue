...
<script>
  ...
  import {JUDGE_STATUS, CONTEST_STATUS, buildProblemCodeKey, CONSTANTS_TEMPLATE} from '@/utils/constants'
    ...
    methods: {
      ...
      init () {
        ...
        api[func](ID).then(res => {
          ...
          problem.language = problem.language.split(',').sort()
          ...
          if (template) {
            Object.keys(template).forEach(res => {
              ...
              firstLanguage = !firstLanguage ? nowName : firstLanguage 
              firstCode = !firstCode ? nowValue.value : firstCode
            })
          }
          problem.codeTemplate = templateLists
          let samples = []
          let inputSamples = problem.inputSamples.substring(1, problem.inputSamples.length - 1).split(',')
          let outputSamples = problem.outputSamples.substring(1, problem.outputSamples.length - 1).split(',')
          let samplesLength = Math.min(inputSamples.length, outputSamples.length)
          for (let i = 0; i < samplesLength; ++i) {
            samples.push({
              input: inputSamples[i].substring(1, inputSamples[i].length - 1).replace(/\\n/ig, '<br/>'),
              output: outputSamples[i].substring(1, outputSamples[i].length - 1).replace(/\\n/ig, '<br/>')
            })
          }
      ...
      changePie (problemData) {
        ...
        // let legend = Object.keys(status).map(ele => JUDGE_STATUS[ele].short)
      ...
      submitCode () {
        ...
        // 由于后端提交的时候没有返回标识位, 因此要想判断当前代码执行的状态, 需要在提交之前获取该用户对于该题的最新提交状态, 之后通过定时器就可以进行判断
