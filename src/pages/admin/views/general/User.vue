<template>
  <div class="view">
    ...
    <el-dialog title="Create New Account" :visible.sync="showUserDialog" :close-on-click-modal="false">
      <el-form :model="user" ref="user" :rules="ruleAccount" label-width="120px" label-position="left">
        <el-row :gutter="20">
          ...
          <el-col :span="user.admin_type==='Regular User' ? 12 : 13">
            <el-form-item :label="$t('m.User_New_Password')" prop="password" required>
              <el-input type="password" v-model="user.password" auto-complete="off" :placeholder="$t('m.password')" @input.native="checkPasswordStrength"></el-input>
              <span>
                <el-button 
                :type="passwordStrength === 'High' ? 'info' : passwordStrength === 'Middle' ? 'success' : 'danger'" 
                v-show="passwordStrength"
                circle>
                  {{ passwordStrength === 'High' ? '强' : passwordStrength === 'Middle' ? '中' : '弱' }}
                </el-button>
              </span>
            </el-form-item>
          </el-col>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ruleAccount: {
          password: {required: true, min: 6, max: 15, message: 'Password Description is required', trigger: 'blur'},
          username: {required: true, min: 2, max: 15, message: 'Username Description is required', trigger: 'blur'}
        },
        passwordStrength: '',
        // 一页显示的用户数
        pageSize: 10,
      ...
      checkPasswordStrength () {
        ...
        for (let i = length - 1; i >= 0; --i) {
          ...
          if (reg.test(password)) {
            this.passwordStrength = nowReg.KEY
            return
          }
        }
