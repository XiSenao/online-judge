<template>
	<div class="authenticated-user">
      ...
      <div class="list">
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-switch v-model="scope.row.status"
                         active-text=""
                         inactive-text=""
                         @change="handleExchangeStatus(scope.row)">
              </el-switch>
            </template>
          </el-table-column> -->
					<el-table-column 
		...
		<el-dialog class="s-dialog" title="Authentication Type" :visible.sync="showAuthenticationTypeDialog" @open="onOpenEditDialog" :close-on-click-modal="false">
				...
				if (!newValue) {
					this.$confirm('Are you sure you want to save these tags?', 'Warning', {
						...
						Promise.all(updateTypeNameQueue).then(res => {
							let saveList = this.authenticationTypeList.filter(res => {
								return res.status === 1
							}).map(res => {
								return res.name
							})
