<template>
  <div class="authenticated-user">
    <Panel :title="!contestID ? 'Authenticated User' : 'Authenticated Contest User'">
      <div v-if="!contestID" slot="header">
      ...
      <div class="list">
        <el-table
          ...
          <el-table-column
            ...
            label="Operation">
            <template slot-scope="scope">
              <icon-btn v-show="scope.row.status === 0 || scope.row.status === -1" name="同意" icon="check-square-o" @click.native="handleExchangeStatus(scope.row, 1)"></icon-btn>
              <icon-btn v-show="scope.row.status === 0 || scope.row.status === 1" name="拒绝" icon="times" @click.native="handleExchangeStatus(scope.row, -1)"></icon-btn>
            </template>
          </el-table-column>
          <el-table-column
            v-if="contestID"
    ...
    'showAuthenticationTypeDialog' (newValue) {
      if (!newValue) {
        this.$confirm('Are you sure you want to save these tags?', 'Warning', {
          ...
          const updateTypeNameQueue = []
          for (let i = 0; i < this.typeSavaList.length; ++i) {
            const nowValue = this.typeSavaList[i]
            const data = {
              id: nowValue.id,
              name: nowValue.name
            }
          ...
          Promise.all(updateTypeNameQueue).then(res => {
            const saveList = this.authenticationTypeList.filter(res => res.status === 1).map(res => res.name)
  ...
  methods: {
    ...
    getContestAuthenticatedUserList () {
      ...
      api.getContestAuthenticatedUserList(params, data).then(res => {
        this.loading = false
        const nowValue = res.data.data.records
        for (const value of nowValue) {
